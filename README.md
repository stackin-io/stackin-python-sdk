...
Human Python SDK for [invoice-api](../invoice_api) — a handful of business fields, nothing about certificates, XML, XSD, signing or SOAP. The API resolves all of that from the issuer's own configuration, identified by `api_key`.
...
Get `api_key` from `POST /api/v1/companies` (or `POST /api/v1/companies/{id}/api-key/rotate`) on invoice-api — one key per issuing company, shown once at creation. invoice-api resolves the issuer (CNPJ, state, address, certificate, environment) entirely from it; nothing about the issuer is ever passed on a call.

```python
from invoice import Invoice, DocumentType, Address
from invoice.br import Product  # Brazil-specific — NCM/CFOP

client = Invoice(base_url="https://invoice-api.example.com", api_key="COMPANY_API_KEY")
...
`extra` accepts `Product` (`invoice.br`) for NFE — required, with `ncm`/`cfop` set — and is ignored for NFSE (a service isn't a physical good, no NCM/CFOP).

## Errors

- `invoice.APIError` — the API responded with a non-2xx status (`status_code`, `detail`) — a 401 here means `api_key` is missing, wrong, or was rotated.
...
Building the full fiscal document (issuer data, service code, tax groups, schema-accurate XML) is [invoice-api](../invoice_api)'s job — configured once per company via `POST /companies`, not passed on every call.
